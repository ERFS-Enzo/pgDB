...
SELECT c.nome, c.cidade, v.modelo, v.marca, v.placa
FROM clientes c, veiculos v
WHERE c.cpf = v.cpf_cli
ORDER BY c.cidade;

SELECT c.nome,c.cidade,v.modelo,v.marca, c.estado
FROM clientes c,veiculos v
WHERE (c.cpf = v.cpf_cli)
AND (c.estado = 'SP');

SELECT c.nome, c.cpf, v.marca, v.modelo
FROM clientes c, veiculos v
WHERE c.cpf = v.cpf_cli 
AND v.marca = 'TOYOTA';

SELECT v.chassi, v.placa, v.modelo, v.marca, c.nome, c.cpf
FROM clientes c, veiculos v
WHERE c.cpf = v.cpf_cli 
AND c.estado != 'SP';

SELECT * FROM veiculos
WHERE marca IN ('TOYOTA');
